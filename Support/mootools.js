...
var MooTools={version:"1.2dev",build:"1535"};var Native=function(J){J=J||{};var F=J.afterImplement||function(){};var G=J.generics;G=(G!==false);var H=J.legacy;
...
var Event=new Native({name:"Event",initialize:function(A,F){F=F||window;A=A||F.event;if(A.$extended){return A;}this.$extended=true;var J=A.type;var G=A.target||A.srcElement;
while(G&&G.nodeType==3){G=G.parentNode;}if(J.test(/key/)){var B=A.which||A.keyCode;var L=Event.Keys.keyOf(B);if(J=="keydown"){var D=B-111;if(D>0&&D<13){L="f"+D;
}}L=L||String.fromCharCode(B).toLowerCase();}else{if(J.match(/(click|mouse|menu)/i)){var I={x:A.pageX||A.clientX+F.document.documentElement.scrollLeft,y:A.pageY||A.clientY+F.document.documentElement.scrollTop};
...
B.erase("Camels");})(Element.Attributes);function memfree(A){if(!A||!A.tagName){return ;}if(Browser.Engine.trident&&(/object/i).test(A.tagName)){for(var C in A){var B=A[C];
if(typeof B=="function"){B=$empty;}}Element.dispose(A);}if(A.removeEvents){A.removeEvents();}}window.addListener("unload",function(){document.purge();if(Browser.Engine.trident){CollectGarbage();
...
F[E]={};F.borderWidth[D]=F[E][D]=B[D]="@px";F.borderStyle[A]=F[E][A]=B[A]="@";F.borderColor[C]=F[E][C]=B[C]="rgb(@, @, @)";});(function(){var A={borders:function(C){if(B.body(C)){return{x:0,y:0};
}return(Browser.Engine.trident)?{x:C.clientLeft,y:C.clientTop}:{x:A.style(C,"border-left-width"),y:A.style(C,"border-top-width")};},parent:function(C){if(B.body(C)){return null;
}if(!Browser.Engine.trident){return $(C.offsetParent);}while((C=C.parentNode)){if(B.positioned(C)){return $(C);}}return null;},style:function(D,C){return Element.getComputedStyle(D,C).toInt()||0;
},margins:function(C){return{x:A.style(C,"margin-left"),y:A.style(C,"margin-top")};},offsets:function(C){return{x:C.offsetLeft,y:C.offsetTop};},size:function(C){return{x:C.offsetWidth,y:C.offsetHeight};
},full:function(C){return{x:C.scrollWidth,y:C.scrollHeight};},scrolls:function(C){return{x:C.scrollLeft,y:C.scrollTop};}};var B={body:function(D){var C=D.tagName.toLowerCase();
return !!(C=="body"||C=="html");},positioned:function(C){return(Element.getComputedStyle(C,"position")!="static");}};Element.implement({positioned:function(){if(B.body(this)){return true;
}return B.positioned(this);},getOffsetParent:function(){return A.parent(this);},getSize:function(){if(B.body(this)){return this.getWindow().getSize();}return A.size(this);
},getScrollSize:function(){if(B.body(this)){return this.getWindow().getScrollSize();}return A.full(this);},getScroll:function(){if(B.body(this)){return this.getWindow().getScroll();
}return A.scrolls(this);},scrollTo:function(C,D){if(B.body(this)){this.getWindow().scrollTo(C,D);}else{this.scrollLeft=C;this.scrollTop=D;}},getPosition:function(G,D){if(B.body(this)){return{x:0,y:0};
}var E=this,C=A.offsets(E);while((E=E.offsetParent)){var I=A.borders(E),F=A.offsets(E);C.x+=F.x+I.x;C.y+=F.y+I.y;}if(G===true){var H=A.borders(this);C.x+=H.x;
C.y+=H.y;}G=$(G);var J=(G)?G.getPosition(true):{x:0,y:0};return{x:C.x-J.x,y:C.y-J.y};},getCoordinates:function(E){if(B.body(this)){return this.getWindow().getCoordinates();
}var C=this.getPosition(E),D=A.size(this);var F={left:C.x,top:C.y,width:D.x,height:D.y};F.right=F.left+F.width;F.bottom=F.top+F.height;return F;},getRelativePosition:function(){return this.getPosition(A.parent(this));
},computePosition:function(D){var C=A.margins(this);return{left:D.x-C.x,top:D.y-C.y};},position:function(C){return this.setStyles(this.computePosition(C));
}});})();Native.implement([Window,Document],{getSize:function(){var D=this.getDocument(),C=this.getWindow(),B=D.documentElement,A=D.body;if(Browser.Engine.webkit419){return{x:C.innerWidth,y:C.innerHeight};
}if(Browser.Engine.presto925){return{x:A.clientWidth,y:A.clientHeight};}return{x:B.clientWidth,y:B.clientHeight};},getScroll:function(){var B=this.getWindow(),A=this.getDocument().documentElement;
return{x:$pick(B.pageXOffset,A.scrollLeft),y:$pick(B.pageYOffset,A.scrollTop)};},getScrollSize:function(){var C=this.getDocument(),B=C.documentElement,A=C.body;
if(Browser.Engine.trident){return{x:Math.max(B.clientWidth,B.scrollWidth),y:Math.max(B.clientHeight,B.scrollHeight)};}if(Browser.Engine.webkit){return get.full(A);
}return get.full(B);},getPosition:function(){return{x:0,y:0};},getCoordinates:function(){var A=this.getSize();return{top:0,left:0,bottom:A.y,right:A.x,height:A.y,width:A.x};
...
},complete:function(){return(!this.stopTimer())?this:this.onComplete();},cancel:function(){return(!this.stopTimer())?this:this.onCancel();},onStart:function(){this.fireEvent("onStart",this.subject);
